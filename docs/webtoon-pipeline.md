...
- **État au 20 septembre 2026.** Règles Firestore déployées sur `lostgarden-e5c3c`, Google actif, `lostgarden.world` et `www.lostgarden.world` autorisés, `AI_GATEWAY_API_KEY` posée sur Vercel (production, preview, development) et dans `.env.local`. **Storage n'est pas encore créé** : le projet est au plan Spark sans compte de facturation, et Firebase exige le plan Blaze pour créer le bucket (`storage.rules` ne se déploie pas tant qu'il n'existe pas). Tant que c'est le cas, une image générée ou remplacée reste dans la session du navigateur et le brouillon ne peut pas la garder (limite d'un document Firestore). Pour débloquer : lier le compte de facturation BUSINESSDYNAMITE au projet dans la console (Paramètres → Utilisation et facturation → Modifier le forfait), puis « Commencer » sur Storage, puis `firebase deploy --only storage`.
