...
export const aiAnimeArticleJa: AiAnimeArticle = {
  ...
  productionParagraphs: [
    ...
    "画像と動画の生成はImaginodeで動いています。Frank Houbreが自ら作ったAI制作プラットフォームで、多数の画像モデルと動画モデルを1つのノードキャンバスの裏にまとめています。おかげでツールを変えずに同じカットを複数のモデルで試せて、キャラクターのリファレンスがそれを使うカットの隣に置かれたままになります。これは単一モデルの選択より重要です。Lanterneの鎧とRoseの顔を1話のあいだ安定させるのは、プロンプトの問題ではなくパイプラインの問題だからです。",
