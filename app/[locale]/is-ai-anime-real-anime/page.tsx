...
const SLUG = "/is-ai-anime-real-anime" as const;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildPageMetadata({
    locale,
    title: dict.meta.isAiAnimeRealAnime.title,
    description: dict.meta.isAiAnimeRealAnime.description,
...
export default async function IsAiAnimeRealAnimePage({ params }: PageProps) {
  const { locale: localeParam } = await params;
  ...
  const media = getArticleMedia(SLUG);
  const page = dict.guides.isAiAnimeRealAnime;
