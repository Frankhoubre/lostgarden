...
const SLUG = "/ai-film-festivals-animation" as const;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildPageMetadata({
    locale,
    title: dict.meta.aiFilmFestivalsAnimation.title,
    description: dict.meta.aiFilmFestivalsAnimation.description,
...
export default async function AiFilmFestivalsAnimationPage({ params }: PageProps) {
  const { locale: localeParam } = await params;
  if (!isLocale(localeParam)) notFound();
  const locale = localeParam as Locale;
  const dict = await getDictionary(locale);
  const guide = getGuide(locale, SLUG);
  const page = dict.guides.aiFilmFestivalsAnimation;
