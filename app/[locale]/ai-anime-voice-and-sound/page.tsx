...
const SLUG = "/ai-anime-voice-and-sound" as const;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildPageMetadata({
    locale,
    title: dict.meta.aiAnimeVoiceAndSound.title,
    description: dict.meta.aiAnimeVoiceAndSound.description,
...
export default async function AiAnimeVoiceAndSoundPage({ params }: PageProps) {
  const { locale: localeParam } = await params;
  if (!isLocale(localeParam)) notFound();
  const locale = localeParam as Locale;
  const dict = await getDictionary(locale);
  const guide = getGuide(locale, SLUG);
  const page = dict.guides.aiAnimeVoiceAndSound;
