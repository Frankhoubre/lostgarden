...
const SLUG = "/making-of-episode-1" as const;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildPageMetadata({
    locale,
    title: dict.meta.makingOfEpisodeOne.title,
    description: dict.meta.makingOfEpisodeOne.description,
...
export default async function MakingOfEpisodeOnePage({ params }: PageProps) {
  const { locale: localeParam } = await params;
  ...
  const media = getArticleMedia(SLUG);
  const page = dict.guides.makingOfEpisodeOne;
