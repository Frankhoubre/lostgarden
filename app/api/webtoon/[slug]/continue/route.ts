...
function writerSystem(input: { script: NonNullable<ReturnType<typeof getWebtoonScript>>; batch: number; characters: string[]; locations: string[] }): string {
  ...
  return [
    ...
    `Answer with JSON only: {"panels": [ {seconds, description, action, emotion, purpose, characters, location, shot_type, camera_angle, composition, narrative_role, transition_type, fidelity, background, dialogue: [{speaker, style, en, fr, anchor}], caption: [{style, en, fr, anchor}], sfx: [{style, en, fr, anchor}]} ] } with exactly ${batch} panels. Escape every double quote inside a string value; no comments, no trailing commas.`,
  ].join("\n\n");
}
