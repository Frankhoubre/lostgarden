...
async function analyzeFrames(input: {
  ...
  const system = [
    ...
    "Two more fields, for every frame. `changed`: what changed since the frame just before it in this list, in a few words: something that appeared or left, grew or formed (a root rising into a bridge), rose, opened, broke, was hit or struck (a mallet on a gong), fell, landed; two creatures fighting; empty for the first frame or when nothing changed. `subtitle`: the subtitle burned into the frame, usually white text at the bottom, copied word for word in its language; empty when there is none. A subtitle is the line actually spoken at that second: never skip one.",
    ...
    'Answer with JSON only: {"frames": [ {seconds, place, characters, helmet, posture, action, in_hands, others_visible, scale, motion, sound, changed, subtitle, title_card} ]}, one entry per frame, in the order given. Escape double quotes inside strings.',
  ]
    .filter(Boolean)
    .join("\n\n");
  ...
  const user: UserPart[] = [
    // The screenplay goes to the writer only: three readers each sent the whole of it cost more than the reading itself.
...
const plain = (text: string) => text.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9 ]+/g, " ").replace(/\s+/g, " ").trim();

/**
 * Checks against the notes that need no model. An object a panel shows but
 * no note shows within two seconds is an invention (a pendant looked at in a
...
function filmChecks(input: { intents: NextPanelIntent[]; notes: FrameNote[]; objects: { id: string; name: string }[]; earlier?: string[] }): { missing: { seconds: number; moment: string }[]; invented: { index: number; problem: string }[] } {
  ...
  const noteText = (seconds: number) =>
    input.notes
      ...
      .map((n) => `${n.in_hands ?? ""} ${n.others_visible ?? ""} ${n.action ?? ""} ${n.changed ?? ""}`)
      .join(" ")
      .toLowerCase();
  input.intents.forEach((intent, index) => {
    const around = noteText(Number(intent.seconds));
    const text = `${intent.description} ${intent.action ?? ""}`.toLowerCase();
    const kept: string[] = [];
    for (const id of intent.objects ?? []) {
      const object = input.objects.find((o) => o.id === id.replace(/^obj\./, ""));
      const words = keywordsOf(id, object?.name ?? id);
      if (words.some((w) => around.includes(w))) kept.push(id);
  ...
  for (const note of [...input.notes].sort((a, b) => Number(a.seconds) - Number(b.seconds))) {
    ...
    const found = lettered.some((l) => words.filter((w) => l.includes(w)).length >= Math.max(1, Math.ceil(words.length * 0.6)));
    if (!found) missing.push({ seconds: Number(note.seconds), moment: `the line "${note.subtitle}" is spoken at ${note.seconds} s (a subtitle of the film): a speech bubble with it, in the panel of that second, the speaker being who says it` });
  }
  return { missing, invented };
}

/**
 * The place of a panel follows the frames: a panel whose location differs
 * from the panels on both sides, at a second where the supervisor saw the
 * same place as its neighbours, takes their location (a forest panel in the
 * middle of a cavern sequence painted the wrong background).
 */
function smoothLocations(intents: NextPanelIntent[], notes: FrameNote[]): void {
  const placeAt = (seconds: number) => {
    const note = notes.reduce<FrameNote | null>((best, n) => (!best || Math.abs(Number(n.seconds) - seconds) < Math.abs(Number(best.seconds) - seconds) ? n : best), null);
    return plain(String(note?.place ?? "")).split(" ").slice(-2).join(" ");
  };
  for (let i = 1; i < intents.length - 1; i += 1) {
    const before = intents[i - 1].location;
    const after = intents[i + 1].location;
    const here = intents[i].location;
    if (!before || before !== after || here === before || intents[i].title_card) continue;
    if (placeAt(Number(intents[i].seconds)) === placeAt(Number(intents[i - 1].seconds))) intents[i].location = before;
  }
}
...
function writerSystem(input: { script: WebtoonScript; lostGarden: boolean; batch: number; characters: string[]; locations: string[]; objects: string[]; pace?: "calm" | "normal" | "action" }): string {
  ...
  return [
    ...
    "Lettering: dialogue from the subtitles of the moments (always), or when the screenplay has a line at that moment, written in English in `en` and in French in `fr`, spoken and short; `style` speech, whisper, thought, shout or off; `speaker` is the character's name. Captions are rare (narration, a place, a time). Sound effects (`sfx`, style soft, hard or rumble) as an English onomatopoeia in `en` and a French one in `fr`, with `size` (96 normal, 180 to 320 for an impact) and `rotate`. Anchors are percentages of the panel: `anchor: {x, y}`.",
