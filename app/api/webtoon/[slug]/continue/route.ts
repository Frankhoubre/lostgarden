...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  const system = [
    ...
    "Write `description` as one or two precise sentences of what the panel shows (subject, pose, framing, light), `action` as the movement or its absence, `emotion` in a few words, `composition` as where the eye goes, `purpose` as why the panel exists. Never use an em dash.",
